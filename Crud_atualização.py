...
# Janela para calculo de IMC  

def janela_calculo_imc():
    """Cria a janela de cálculo de IMC"""
    janela_imc = tk.Toplevel()
    janela_imc.title("Calculadora de IMC")
    janela_imc.geometry("400x400")
    
    # Frame principal
    frame_principal = ttk.Frame(janela_imc, padding="20")
    frame_principal.pack(fill=tk.BOTH, expand=True)
    
    # Entradas
    ttk.Label(frame_principal, text="Peso (kg):").grid(row=0, column=0, sticky=tk.W, pady=5)
    peso_entry = ttk.Entry(frame_principal)
    peso_entry.grid(row=0, column=1, pady=5)
    
    ttk.Label(frame_principal, text="Altura (m):").grid(row=1, column=0, sticky=tk.W, pady=5)
    altura_entry = ttk.Entry(frame_principal)
    altura_entry.grid(row=1, column=1, pady=5)
    
    # Resultados
    resultado_frame = ttk.Frame(frame_principal)
    resultado_frame.grid(row=2, column=0, columnspan=2, pady=20)
    
    ttk.Label(resultado_frame, text="Seu IMC:").pack()
    imc_label = ttk.Label(resultado_frame, text="", font=('Arial', 14, 'bold'))
    imc_label.pack()
    
    ttk.Label(resultado_frame, text="Classificação:").pack()
    classificacao_label = ttk.Label(resultado_frame, text="", font=('Arial', 12))
    ...
    def calcular_imc():
        try:
            ...
            imc = peso / (altura ** 2)
            imc_label.config(text=f"{imc:.1f}")
            ...
            classificacao_label.config(text=classificacao)
            
        except ValueError:
            messagebox.showerror("Erro", "Digite valores numéricos válidos!")
    
    # Botão de cálculo
    ttk.Button(frame_principal, text="Calcular IMC", command=calcular_imc).grid(row=3, column=0, columnspan=2, pady=10)
    ttk.Button(frame_principal, text="Fechar", command=janela_imc.destroy).grid(row=4, column=0, columnspan=2)
...
def menu_principal():
    ...
    # Botão 2 (ENTRAR) e 3 (CALCULAR IMC) - Amarelo personalizado
    ...
    try:
        ...
        # Frame transparente para os botões (usando tk.Frame em vez de ttk para transparência)
        main_content_frame = tk.Frame(janela, bg='', bd=0) 
        ...
        main_content_frame.pack(side=tk.BOTTOM, pady=80) # bg vazio para transparência
        main_content_frame.place(relx=0.5, rely=0.8, anchor='center')

    except FileNotFoundError:
        messagebox.showwarning("Aviso", f"Imagem de fundo não encontrada em: {BACKGROUND_IMAGE_PATH}")
        janela.geometry("800x600")
        main_content_frame = ttk.Frame(janela)
    ...
    # Botões (agora com fundo transparente sobre a imagem)
    btn_inscreva = ttk.Button(main_content_frame, text="INSCREVA-SE HOJE", 
                             command=lambda: janela_cadastro("Aluno"), 
                             style='YellowButton.TButton',
                             width=25)
    btn_inscreva.pack(pady=10)

    btn_entrar = ttk.Button(main_content_frame, text="ENTRAR", 
                          command=janela_entrar,
                          style='DarkButton.TButton',
                          width=20)
    btn_entrar.pack(pady=5)

    btn_imc = ttk.Button(main_content_frame, text="CALCULAR IMC", 
                       command=janela_calculo_imc,
                       style='DarkButton.TButton',
                       width=20)
    btn_imc.pack(pady=5)
